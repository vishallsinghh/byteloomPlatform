...
function GalleryPage() {
  ...
  const OTHER_COLORS = [
    'rgb(165, 148, 249)', 'rgb(176, 219, 156)', 'rgb(255, 128, 128)', 'rgb(148, 181, 249)', 'rgb(185, 178, 219)', 'rgb(249, 165, 148)',
    'rgb(247, 207, 216)', 'rgb(255, 199, 133)', 'rgb(163, 216, 255)', 'rgb(185, 243, 252)', 'rgb(174, 226, 255)', 'rgb(147, 198, 231)',
    'rgb(254, 222, 255)', 'rgb(244, 191, 191)', 'rgb(255, 217, 192)', 'rgb(250, 240, 215)', 'rgb(140, 192, 222)', 'rgb(216, 148, 249)'
  ]
  ...
  const handleDelete = chartId => {
    Swal.fire({
      ...
      try {
        ...
        Swal.fire('Error', 'There was a problem deleting your chart. Please try again.', 'error')
      }
    })
  }
    ...
    const makeSeries = () => {
      return data.datasets.map((ds, i) => {
        ...
        if (type === 'scatter') {
          ...
          const pts = ds.data.map((value, idx) => {
      ...
      <div className="h-[calc(100vh-55px)] mt-[55px] flex overflow-hidden">
        ...
        <aside className="w-[60px] h-[calc(100vh-55px)] fixed flex flex-col items-center py-4 space-y-4 border-r border-gray-200 bg-white z-20">
          ...
          <div className='flex flex-col items-center'>
            <button
              onClick={() => {
                if (selectedId) {
                  navigate(`/chart?datasetId=${encodeURIComponent(selectedId)}`);
                } else {
                  Swal.fire('Info', 'Please select a dataset first.', 'info')
                }
              }}
              className={`p-2 rounded hover:bg-gray-200 ${!selectedId ? 'opacity-50 cursor-not-allowed' : ''}`}
        ...
        <main className="ml-[60px] pt-6 flex flex-col h-full bg-[#F4F5F9] w-full overflow-y-auto">
          {!selectedId ? (
            ...
            <>
              {/* KPIs */}
              <h2 className="text-3xl font-bold mb-3 text-gray-700 px-6">KPIs</h2>
              ...
              {/* Charts grid */}
              <h2 className="text-3xl font-bold mb-3 text-gray-700 px-6">Charts</h2>
                  ...
                  charts.length > 0 ? charts.map((chart, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="absolute top-2 right-2 flex items-center gap-2 z-10">
                          <button onClick={() => handleEdit(chart)} className="p-1 hover:text-blue-600">
                            <FiEdit size={18} />
                          </button>
                          <button onClick={() => handleDelete(chart.chart_id)} className="p-1 hover:text-red-600">
                            <FiTrash2 size={18} />
                          </button>
                        </div>
                    ...
                    <div className="col-span-full border rounded-lg p-10 bg-white text-center text-gray-500">
                      ...
                      <button
                        ...
                        className={`p-2 px-3 gap-3 mt-4 bg-pink-600 text-white hover:bg-pink-700 hover:drop-shadow-md mx-auto rounded flex items-center border ${!selectedId ? 'opacity-50 cursor-not-allowed' : ''}`}
                        title={selectedId ? "Create New Chart" : "Select a dataset first"}
                        disabled={!selectedId}
                      >
